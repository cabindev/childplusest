// app/news/components/share-buttons.tsx
'use client';
...
export default function ShareButtons({ title, url }: ShareButtonsProps) {
  ...
  // ใช้ current URL ถ้าไม่ได้ส่ง url มา
  ...
  return (
    <div className="flex items-center space-x-2">
      <span className="text-sm text-gray-600 mr-2">แชร์:</span>
      
      {shareButtons.map((button) => (
        <button
          key={button.name}
          onClick={() => handleShare(button.url)}
          className={`${button.color} text-white p-2 rounded-full transition-colors duration-200 hover:scale-105 transform`}
          title={`แชร์ใน ${button.name}`}
        >
          {button.icon}
        </button>
      ))}
      ...
      <button
        ...
        } text-white p-2 rounded-full transition-all duration-200 hover:scale-105 transform`}
        title="คัดลอกลิงก์"
      >
        {copied ? (
